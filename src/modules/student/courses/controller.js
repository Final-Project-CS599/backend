...
export const storePaymentData = asyncHandler(async (req, res) => {
  ...
  const insertPaymentQuery = `
      ...
      VALUES (?, NOW(), ?, NULL)
    `;
  const [paymentResult] = await dbConfig.promise().query(insertPaymentQuery, [filePath, studentId]);

  const paymentId = paymentResult.insertId;

  // Insert into extra_payment table to link payment to course
  const insertExtraPaymentQuery = `
      INSERT INTO extra_payment (student_id, course_id, payment_id)
      VALUES (?, ?, ?)
    `;
  await dbConfig.promise().query(insertExtraPaymentQuery, [studentId, course_id, paymentId]);
  ...
  return res.status(200).json({
    ...
    filePath,
    course_id,
    payment_id: paymentId, // Return the payment_id for reference
  });
});
