...
const addStudent = errorAsyncHandler(async (req, res, next) => {
  const {
    admin_nationalID,
    firstName,
    lastName,
  ...
  dbConfig.execute(
    ...
    (err, adminData) => {
      ...
      if (!adminData.length) {
        return next(
          new Error('Access denied, Only admins or superAdmins can add instructors', { cause: 403 })
        );
      ...
      dbConfig.execute(`select * from student where s_email =?`, [email], (err, data) => {
        if (err) {
          return res
            .status(500)
            .json({ message: 'Failed to get data , Faik to execute query', error: err });
        } else if (data.length) {
          return res
            .status(409)
            .json({ message: 'Student email already exists', email: data[0].s_email });
        } else {
          dbConfig.execute(
            `select * from student where s_national_id =?`,
            [numberNational],
            (err, data) => {
              if (err) {
                return res
                ...
                return res.status(409).json({
                  message: 'Student numberNational already exists',
                  numberNational: data[0].s_national_id,
                });
              } else {
                  ...
                  async (err, data) => {
                    if (err || !data.affectedRows === 0) {
                      return res.status(500).json({
                        message: 'Failed to get data , Faik to execute query',
                        error: err,
                      });
                    }
                    ...
                    try {
                      ...
                      const addedPhones = await Promise.all(
                      ...
                      return successResponse({
                        res,
                        message: 'Instructor and phone added successfully',
                        status: 201,
                        data: {
                          admin: {
