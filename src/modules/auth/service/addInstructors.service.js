...
const addInstructor = errorAsyncHandler(async (req, res, next) => {
  const {
    admin_nationalID,
    firstName,
    lastName,
        ...
        if (err) {
          ...
          dbConfig.execute(
            ` insert into Instructors (
                            ...
                            values (?,?,?,?,?) `,
            [firstName, lastName, email, hashPassword, admin_nationalID],
            async (err, data) => {
              if (err || !data.affectedRows === 0) {
                return next(
                  new Error('Failed to add data , Fail to execute query', { cause: 500 })
                );
