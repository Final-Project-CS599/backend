...
const addAdmin = errorAsyncHandler(async (req, res, next) => {
  const {
    adminNationalID,
    firstName,
    lastName,
  ...
  dbConfig.execute(
    ` SELECT * FROM superAdmin WHERE sAdmin_nationalID = ? AND sAdmin_role = 'sAdmin' `,
    [sAdminNationalID],
    (err, dataSuperAdmin) => {
      ...
      dbConfig.execute(
        `SELECT * FROM superAdmin WHERE sAdmin_nationalID = ? OR sAdmin_email = ?`,
        ...
        (err, dataAdmin) => {
          ...
          dbConfig.execute(
            `INSERT INTO superAdmin(
                            ...
                            VALUES (?,?,?,?,?,?)`,
            [adminNationalID, firstName, lastName, email, hashPassword, adminRole],
            async (err, data) => {
              if (err || !data.affectedRows === 0) {
                return res
                  .status(500)
                  .json({ message: 'Failed to get data , Fail to execute query', error: err });
              }
              ...
              try {
                ...
                return res
                  .status(500)
                  .json({ message: 'Failed to insert phone numbers', error: phoneError });
              }
            }
          );
        }
