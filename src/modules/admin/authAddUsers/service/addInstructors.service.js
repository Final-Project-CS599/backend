...
const addInstructor = errorAsyncHandler(async (req, res, next) => {
    ...
    dbConfig.execute(`SELECT * FROM superAdmin WHERE sAdmin_nationalID = ?`,
        ...
        async (err, adminData) => {
            if (err) {
                return next(new Error("Error Server Database verify admin/superAdmin role", { cause: 500 }));
            }
            if (!adminData.length) {
            ...
            try {
                ...
                dbConfig.execute(`INSERT INTO Instructors (
                    ...
                    [firstName, lastName, email, hashPassword, departmentId, admin_nationalID],
                    async (err, data) => {
                        if (err) {
                ...
                return next(new Error(`Error Server Database Failed to get data check email exist `, { cause: 500 }));
            }
        }
    );
});
