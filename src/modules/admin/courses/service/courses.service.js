...
export const addCourses = errorAsyncHandler(async (req, res, next) => {
  const {
    adminNid,
    courseName,
    courseType,
        ...
        (err, data) => {
          ...
          dbConfig.execute(
            ...
            (err, data) => {
              ...
              if (data.length > 0) {
                return next(
                  new Error('Course , Instructor  and date already exists ', { cause: 409 })
                );
              }
...
export const deletedCourses = errorAsyncHandler(async (req, res, next) => {
  ...
  dbConfig.execute(`SELECT * FROM courses WHERE c_name = ?`, [courseName], (err, courseData) => {
    if (err) {
      return next(new Error('Error Server', { cause: 500 }));
    }
